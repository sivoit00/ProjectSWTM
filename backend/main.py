...
def update_werkstatt(werkstatt_id: int, werkstatt: schemas.WerkstattCreate, db: Session = Depends(get_db)):
    ...
    if not db_werkstatt:
        raise HTTPException(status_code=404, detail="Werkstatt nicht gefunden")
...
def delete_fahrzeug(fahrzeug_id: int, db: Session = Depends(get_db)):
    ...
    if not db_fahrzeug:
        raise HTTPException(status_code=404, detail="Fahrzeug nicht gefunden")
    db.delete(db_fahrzeug)
    db.commit()
    return {"message": f"Fahrzeug {fahrzeug_id} gelöscht"}
...
def delete_werkstatt(werkstatt_id: int, db: Session = Depends(get_db)):
    ...
    if not db_werkstatt:
        raise HTTPException(status_code=404, detail="Werkstatt nicht gefunden")
    db.delete(db_werkstatt)
    db.commit()
    return {"message": f"Werkstatt {werkstatt_id} gelöscht"}
...
def delete_auftrag(auftrag_id: int, db: Session = Depends(get_db)):
    ...
    if not db_auftrag:
        raise HTTPException(status_code=404, detail="Auftrag nicht gefunden")
    db.delete(db_auftrag)
    db.commit()
    return {"message": f"Auftrag {auftrag_id} gelöscht"}
